...
export module Lwg_Task {
    ...
    * */
    ...
    export function _doDetection(calssName: string, name: string, number?: number): any {
        ...
        let resCondition = Lwg_Task._getProperty(calssName, name, Lwg_Task._Property.resCondition);
        let condition = Lwg_Task._getProperty(calssName, name, Lwg_Task._Property.condition);
        if (Lwg_Task._getProperty(calssName, name, Lwg_Task._Property.get) !== -1) {
            if (condition <= resCondition + number) {
                Lwg_Task._setProperty(calssName, name, Lwg_Task._Property.resCondition, condition);
                Lwg_Task._setProperty(calssName, name, Lwg_Task._Property.get, 1);
                if (_TaskList) {
                    _TaskList.refresh();
                }
                return true;
            } else {
                Lwg_Task._setProperty(calssName, name, Lwg_Task._Property.resCondition, resCondition + number);
                if (_TaskList) {
                    _TaskList.refresh();
                }
                return false;
    ...
    export function _init(): void {
        ...
        if (_todayDate.date !== (new Date).getDate()) {
            Lwg_Task._everydayTask = Laya.loader.getRes('LwgGameData/Task/_everydayTask.json')['RECORDS'];
            console.log('不是同一天，每日任务重制！');
            _todayDate.date = (new Date).getDate();
        } else {
            Lwg_Task._everydayTask = Tools.jsonCompare('LwgGameData/Task/_everydayTask.json', _Classify.everyday, _Property.name);
    ...
    export class _TaskScene extends Admin._Scene {
        moduleOnAwake(): void {
            _allClassifyArr = [Lwg_Task._everydayTask];
            Lwg_Task._TaskTap = this.self['TaskTap'];
            Lwg_Task._TaskList = this.self['TaskList'];
        }
        moduleOnEnable(): void {
            this.lwgTapCreate();
            this.lwgListCreate();
        ...
        /**Tap初始化*/
        lwgTapCreate(): void {
            Lwg_Task._TaskList.selectHandler = new Laya.Handler(this, this.lwgTapSelect);
        }
        /**taskTap的触摸监听,重写覆盖*/
        lwgTapSelect(index: number): void { }
        /**初始化list*/
        lwgListCreate(): void {
            Lwg_Task._TaskList.selectEnable = true;
            Lwg_Task._TaskList.vScrollBarSkin = "";
            // this._ShopList.scrollBar.elasticBackTime = 0;//设置橡皮筋回弹时间。单位为毫秒。
            // this._ShopList.scrollBar.elasticDistance = 500;//设置橡皮筋极限距离。
            Lwg_Task._TaskList.selectHandler = new Laya.Handler(this, this.lwgListScelet);
            Lwg_Task._TaskList.renderHandler = new Laya.Handler(this, this.lwgListUpdate);
            if (_allClassifyArr[0]) {
                Lwg_Task._TaskList.array = _allClassifyArr[0];
        ...
        lwgAddItemComponent(): void {
            for (let index = 0; index < Lwg_Task._TaskList.cells.length; index++) {
                const element = Lwg_Task._TaskList.cells[index];
...
export default class UITask extends Lwg_Task._TaskScene {
}
