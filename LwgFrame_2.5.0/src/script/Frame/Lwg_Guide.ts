...
export module Lwg_Guide {
    ...
    export class GuideScene extends Admin._Scene {
        // moduleOnAwake(): void {
        // }
        // moduleOnEnable(): void {
        // }
...
export default class UIGuide extends Lwg_Guide.GuideScene {

    lwgOnEnable(): void {
        this.self['Background'].alpha = 0;
        this.self['Hand'].alpha = 0;
        EventAdmin.notify(Lwg_Guide.EventType.onStep);
    ...
    lwgEventReg(): void {
        ...
        var step1 = () => {
            ...
            EventAdmin.notify(Lwg_Guide.EventType.appear);
        ...
        // 执行
        EventAdmin.reg(Lwg_Guide.EventType.onStep, this, () => {
            Laya.timer.once(500, this, () => {
                console.log('新手引导到了第：', Lwg_Guide._whichStepNum + '步了');
                switch (Lwg_Guide._whichStepNum) {
                    case 1:
                        step1();
                        break;
                    case 2:
        ...
        EventAdmin.reg(Lwg_Guide.EventType.appear, this, (func) => {
            Animation2D.fadeOut(this.self['Hand'], 0, 1, 150);
            Animation2D.fadeOut(this.self['Background'], 0, 0.5, 300);
        })
        EventAdmin.reg(Lwg_Guide.EventType.stepComplete, this, () => {
            Lwg_Guide._whichStepNum++;
            let DrawCanvas = this.self['Hand'].getChildByName('DrawCanvas');
            if (this.self['Hand'].getChildByName('DrawCanvas')) {
                this['drawLinePos'] == false;
                DrawCanvas.removeSelf();
        ...
        EventAdmin.reg(Lwg_Guide.EventType.complete, this, () => {
            Animation2D.fadeOut(this.self['Hand'], 1, 0, 150);
            Animation2D.fadeOut(this.self['Background'], 0.5, 0, 300, 0, () => {
                Lwg_Guide._complete.bool = true;
